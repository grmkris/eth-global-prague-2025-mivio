...
export function useEventChannelSimple(options: UseEventChannelSimpleOptions) {
  ...
  useEffect(() => {
    ...
    const savedChannelId = localStorage.getItem(getStorageKey(eventSlug, 'channelId'));
    if (savedChannelId) {
  ...
  const createChannel = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem(getStorageKey(eventSlug, 'channelId'), result.channelId);
  ...
  }, [participantStats, channelId, isLoading, walletAddress, walletClient, nitroliteClient, createChannel]);
