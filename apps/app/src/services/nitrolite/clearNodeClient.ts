...
export class ClearNodeClient {
  ...
  private pendingRequests = new Map<number, { resolve: (value: any) => void; reject: (reason: Error) => void }>();
  ...
  private async authenticate(): Promise<void> {
    if (!this.ws || !this.authParams) throw new Error('WebSocket not connected or auth params missing');

    const { walletAddress, signerAddress, walletClient } = this.authParams;
    
    // Check for JWT token first
    const jwtToken = typeof window !== 'undefined' ? window.localStorage?.getItem('clearnode_jwt') : null;
      ...
      const handleAuthResponse = async (event: MessageEvent) => {
        ...
        try {
          if (response.res && response.res[1] === 'auth_challenge') {
            ...
            const eip712Signer = async (data: any): Promise<`0x${string}`> => {
            ...
            // Send auth verify
            const authVerify = await createAuthVerifyMessage(eip712Signer, event.data);
            this.ws?.send(authVerify);
          } else if (response.res && response.res[1] === 'auth_success') {
            // Store JWT if provided
            if (response.res[2]?.[0]?.jwt_token && typeof window !== 'undefined') {
              window.localStorage?.setItem('clearnode_jwt', response.res[2][0].jwt_token);
            }
            
            clearTimeout(authTimeout);
            this.ws?.removeEventListener('message', handleAuthResponse);
            resolve();
          } else if (response.err || (response.res && response.res[1] === 'auth_failure')) {
            const errorMsg = response.err?.[2] || response.res?.[2] || 'Authentication failed';
   ...
   */
  async sendRequest(signedRequest: string): Promise<any> {
...
export function getClearNodeClient(): ClearNodeClient {
  if (!clearNodeInstance) {
    clearNodeInstance = new ClearNodeClient({
      url: process.env.NEXT_PUBLIC_CLEARNODE_URL || 'wss://clearnet.yellow.com/ws',
    });
  }
