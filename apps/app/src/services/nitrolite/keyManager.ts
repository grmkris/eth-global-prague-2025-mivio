	...
	try {
		...
		if (savedKeys) {
			try {
				...
				console.error(
					"Failed to parse saved keys, generating new ones:",
					error,
				);
 ...
 */
export const createStateWalletClient = (
			...
			sign: async (payload: RequestData | ResponsePayload): Promise<Hex> => {
				try {
					...
					const { serialized: signature } =
						wallet.signingKey.sign(messageBytes);
...
export const createMessageSigner = (stateWallet: WalletSigner) => {
	return async (props: { payload: unknown; walletClient: WalletClient }) => {
		...
		const signature = await stateWallet.sign(
			payload as RequestData | ResponsePayload,
		);
