...
import { events, eventWallets, transactions, users } from "~/server/db/schema";
...
export const walletRouter = createTRPCRouter({
	...
	getTransactions: publicProcedure
		.input(
			z.object({
				...
				type: z
					.enum(["all", "payment", "reward", "topup", "purchase", "refund"])
					.optional(),
			}),
		)
		.query(async ({ ctx, input }) => {
	...
	topUp: publicProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const newBalance = Number.parseFloat(wallet[0].balance) + input.amount;
			const newTotalEarned =
				Number.parseFloat(wallet[0].totalEarned) + input.amount;
	...
	createPayment: publicProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const newTotalSpent =
				Number.parseFloat(wallet[0].totalSpent) + input.amount;
