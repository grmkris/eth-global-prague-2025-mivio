...
import { tasks, taskCompletions, users, events, eventWallets, transactions, rewards, userRewards } from "~/server/db/schema";
...
export const taskRouter = createTRPCRouter({
  ...
  complete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingCompletion[0] && existingCompletion[0].status === "completed") {
        throw new TRPCError({
          code: "BAD_REQUEST",
      ...
      if (task[0].maxCompletions) {
        ...
        if (completionCount[0] && completionCount[0].count >= task[0].maxCompletions) {
          throw new TRPCError({
            code: "BAD_REQUEST",
      ...
      if (status === "completed" && task[0].rewardType === "token") {
        ...
        if (wallet[0]) {
          ...
          const newBalance = parseFloat(wallet[0].balance) + parseFloat(task[0].reward);
          const newTotalEarned = parseFloat(wallet[0].totalEarned) + parseFloat(task[0].reward);
