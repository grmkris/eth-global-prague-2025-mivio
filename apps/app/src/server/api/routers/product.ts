...
import { products, purchases, users, events, eventWallets, transactions } from "~/server/db/schema";
...
export const productRouter = createTRPCRouter({
  ...
  getByEvent: publicProcedure
    .input(z.object({
      eventSlug: z.string(),
      category: z.enum(["all", "merch", "food", "experience", "ticket"]).optional(),
  ...
  purchase: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const totalPrice = parseFloat(product[0].price) * input.quantity;
      ...
      const newTotalSpent = parseFloat(wallet[0].totalSpent) + totalPrice;
  ...
  getUserPurchases: publicProcedure
    .input(z.object({
      eventSlug: z.string(),
      walletAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/),
      status: z.enum(["all", "pending", "completed", "cancelled", "refunded"]).optional(),
    }))
    .query(async ({ ctx, input }) => {
      // Get user
      const user = await ctx.db
