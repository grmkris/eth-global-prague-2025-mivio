...
import { rewards, userRewards, users, events, tasks, taskCompletions, eventWallets, eventParticipants } from "~/server/db/schema";
          ...
          switch (achievement.criteria.type) {
            case "checkin": {
              ...
              if (participation[0]?.checkedIn && participation[0].checkedInAt) {
                ...
                const diffSeconds = (checkinTime.getTime() - eventStart.getTime()) / 1000;
                earned = diffSeconds <= achievement.criteria.timeLimit!;
            ...
            case "balance": {
              ...
              const totalEarned = parseFloat(wallet[0]?.totalEarned || "0");
              earned = totalEarned >= achievement.criteria.amount!;
              progress = Math.min(
                100,
                Math.round((totalEarned / achievement.criteria.amount!) * 100)
              );
              break;
            }
      ...
      const existing = await ctx.db
        ...
        .where(
          and(
            ...
            input.eventId ? eq(userRewards.eventId, input.eventId) : isNull(userRewards.eventId)
          )
        )
        .limit(1);
