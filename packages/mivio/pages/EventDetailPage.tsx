...
const EventDetailPage: React.FC = () => {
  ...
  const handleUnlockSuccess = () => {
    ...
    if (event) {
      ...
      setEvent((prevEvent) =>
        prevEvent ? { ...prevEvent, isUnlockedByUser: true } : null
      );
    }
  };
  ...
  if (!event) {
    return (
      <div className="p-4 text-center min-h-screen flex flex-col justify-center items-center bg-slate-50">
        ...
        <p className={`${PASTEL_COLORS.textLight} mb-6`}>
          The event you are looking for does not exist or has been moved.
        </p>
        <Link
          to="/events"
          className={`${PASTEL_COLORS.mint.button} text-white py-2 px-4 rounded-lg`}
        >
          Back to Events
        </Link>
      </div>
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      ...
      <div className="p-4 space-y-6">
        ...
        {currentEventState.mapImageUrl && (
          <section className="animated-section-event opacity-0 translate-y-5 transition-all duration-500 ease-out delay-100">
            <div className="bg-white p-5 rounded-2xl shadow-lg">
              ...
              />
              <p
                className={`text-xs ${PASTEL_COLORS.textLight} mt-2 text-center`}
              >
                Abstract representation of venue zones and key points.
              </p>
            </div>
          </section>
        )}
        ...
        {currentEventState.brandActivations &&
          currentEventState.brandActivations.length > 0 && (
            <section className="animated-section-event opacity-0 translate-y-5 transition-all duration-500 ease-out delay-200">
              <h2 className="text-xl font-semibold text-slate-700 mb-3 px-1">
                Brand Activations
              </h2>
              <div className="flex overflow-x-auto space-x-0 pb-2 scrollbar-hide px-1">
                {" "}
                {/* Adjusted space-x for card margins */}
